...
export default function IssueDetailPage() {
  ...
  const handleEdit = async (formData: IssueFormData) => {
    try {
      ...
      toast.error(getApiErrorMessage(err, "Failed to update issue"));
    }
  };
  ...
  return (
    <div className="page-shell mx-auto max-w-4xl">
      ...
      <section className="surface overflow-hidden">
        ...
        <div className="grid gap-4 border-b border-slate-200 px-6 py-5 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <MetaItem
            icon={Clock3}
            label="Updated"
            value={formatDistanceToNow(new Date(issue.updatedAt), {
              addSuffix: true,
            })}
          />
        </div>
      ...
      <section className="surface overflow-hidden">
        ...
        <div className="divide-y divide-slate-200">
          {issue.comments.length === 0 && (
            <p className="px-6 py-8 text-sm text-slate-600">No comments yet.</p>
          )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          deletingComment && handleDeleteComment(deletingComment)
        }
